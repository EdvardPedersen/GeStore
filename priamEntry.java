...
public class priamEntry extends genericEntry{
    ...
    public boolean addEntry(String entry) {
        ...
        if(hdfs != null) {
            try {
                hdfs.rename(sourcePath, new Path(targetPath.toString() + suffix));
            } catch (IOException E) {
        ...
        fieldKeys.put("ID", suffix);
        fieldKeys.put("HASH", pathHashTimestamp[1]);
        fieldKeys.put("TIMESTAMP", pathHashTimestamp[2]);
        fieldKeys.put("LOCATION", targetPath.toString() + suffix);
    ...
    private Text getFileList() {
        Text outText = new Text(fieldKeys.get("ID"));
        return outText;
    }
}